...
csv_data = """name	phone	instagram	cpf	birth_date	size	total_spent	address_street	address_number	address_complement	address_neighborhood	address_city	address_state	address_zip	notes	photo_url	created_at	updated_at
...
Luma Beatriz	11941711996		408.409.988-05	1991-12-09	G	0	RUA PROF JOSÁ MARIA CALAZANS NOGUEIRA	232	Apto 303 A	PQ SÃO DOMINGOS	São Paulo		05122010			2026-02-24 13:47:04.15397+00	2026-02-24 13:47:04.15397+00
...
for i in range(1, len(lines)):
    ...
    name = cols[0].replace("'", "''")
    phone = cols[1].replace("'", "''")
    instagram = cols[2].replace("'", "''")
    document = cols[3].replace("'", "''")
    birth_date = cols[4].strip()
    size = cols[5].replace("'", "''")
    
    try:
        total_spent = float(cols[6]) if cols[6] else 0.0
    except:
        total_spent = 0.0

    addr_parts = [c for c in cols[7:14] if c.strip()]
    address = ", ".join(addr_parts).replace("'", "''")
    
    notes = cols[14].replace("'", "''")
    
    if len(birth_date) == 10 and "-" in birth_date and birth_date[0].isdigit() and birth_date[0] != "0":
        bdate_str = f"'{birth_date}'::date"
    else:
        bdate_str = "NULL"

    sql += f"  INSERT INTO public.customers (owner_id, name, phone, instagram, document, address, birth_date, size_top, training_location)\n"
    sql += f"  VALUES (v_owner, '{name}', '{phone}', '{instagram}', '{document}', '{address}', {bdate_str}, '{size}', '{notes}') RETURNING id INTO v_customer_id;\n"
    
    if total_spent > 0:
        sql += f"  INSERT INTO public.sales (owner_id, customer_id, total_amount, payment_method, status, observations)\n"
        sql += f"  VALUES (v_owner, v_customer_id, {total_spent}, 'legacy_import', 'completed', 'Importação de Histórico: R$ {total_spent}');\n"
    
    sql += "\n"

sql += "END $$;\n"

with open("c:\\Users\\Team WOD Brasil\\Desktop\\Revenda Profit\\import_dayana_clientes.sql", "w", encoding="utf-8") as f:
    f.write(sql)

print("SQL Saved to root")
